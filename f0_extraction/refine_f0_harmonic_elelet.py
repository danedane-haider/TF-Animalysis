...
def refine_f0_file(
    ...
    max_kernel_size = int(max(kernel_sizes))
    edge_frames = int(max_kernel_size // 2 // max(stride, 1))
...
def refine_f0_folder(
    ...
    print(
        "Elelet: "
        f"kernel_sizes={kernel_sizes if kernel_sizes else [kernel_size]}, "
        f"channels={num_channels}, stride={stride}, "
        f"fmin={elelet_fmin}, fmax={elelet_fmax}, supp_mult={supp_mult}, scale={scale}"
    )
    eff_kernel_sizes = kernel_sizes if kernel_sizes else [kernel_size]
    edge_frames = int(max(eff_kernel_sizes) // 2 // max(stride, 1))
    print(
        "Params: "
